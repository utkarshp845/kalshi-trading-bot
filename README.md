...
- **A third-party API can deprecate a working endpoint out from under you with no code change on your end — and the fix for that can introduce a worse bug than the one it solves.** After the maker-fill deadlock fix above shipped and the bot resumed placing real orders, every single one failed with `HTTP 410 Gone` — the legacy `POST /portfolio/orders` order-creation endpoint had been sunset in favor of a V2 endpoint (`POST /portfolio/events/orders`) with a materially different request/response shape (fixed-point decimal strings instead of cents integers, a single YES-side `bid`/`ask` book instead of separate `yes`/`no` sides, new required fields, no `status` field in the create response). Nothing in this repo changed; Kalshi's API did. The bare exception message (`"410 Client Error: Gone for url: ..."`) gave no hint why — logging the response *body*, not just the status line, was what actually revealed the cause (`{"error": {"code": "deprecated_v1_order_endpoint", ...}}`). Migrating to V2 meant re-fetching the newly created order via the (unaffected) legacy `GET /portfolio/orders/{id}` to keep every downstream consumer working unchanged — but that GET turned out to 404 for several real seconds immediately after a genuine, successful create (read-after-write propagation lag, confirmed by re-querying the same order IDs later and finding them `executed`). Treating that verification failure the same as an outright order-creation failure caused a real incident: a maker order that had actually filled looked like a rejection, so the code did exactly what it's designed to do on a real rejection — fall back to a taker order for the same signal — creating a second real fill and doubling the position. Fix required two distinct pieces: retry the post-create GET through the propagation window, and make a verification failure raise a *different, non-retryable* exception type than a genuine creation failure, so the calling code can no longer confuse "I don't know what happened" with "it's safe to try again." Lesson: for any own-action-out, network-response-in verification step, "I couldn't confirm success" and "it failed" are not the same outcome and must not share an exception type or a recovery path — the first demands a loud alert and standing down, not a retry.
