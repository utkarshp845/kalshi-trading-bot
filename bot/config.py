...
THEO_PROB_BAND_MIN: float = _float("THEO_PROB_BAND_MIN", 0.20)
THEO_PROB_BAND_MAX: float = _float("THEO_PROB_BAND_MAX", 0.80)
MIN_SANE_STRIKES: int = _int("MIN_SANE_STRIKES", 2)          # min liquid strikes per expiry chain; was hardcoded 4
MAX_SIGMA_DISTANCE: float = _float("MAX_SIGMA_DISTANCE", 2.0)
...
LIVE_MIN_REQUIRED_EDGE: float = _float("LIVE_MIN_REQUIRED_EDGE", 0.06)  # was 0.10 — 10% still too high for liquid BTC markets
COLD_START_MIN_EDGE: float = _float("COLD_START_MIN_EDGE", 0.06)  # was 0.15 — strict cold-start caused bootstrap deadlock (no fills → no history → no fills)
...
KELLY_FRACTION: float = _float("KELLY_FRACTION", 0.30)    # was 0.20 — at small balances 0.20 rounds to 0 contracts; 0.30 reaches 1 contract at ~$150
